...
\end{lstlisting}
\captionof{lstlisting}{Java source code}
\label{sc:java-code}
\end{minipage}\hfill
\begin{minipage}{.5\textwidth}
...
\end{lstlisting}
\captionof{lstlisting}{Soot Jimple code}
\label{sc:soot-code}
\end{minipage}
\end{figure}
...
\begin{figure}[th]
  \centering
\rel{\_AssignOperFrom}{insn, position, from}.\\
\rel{\_IfVar}{insn, position, var}.
...
\begin{figure}[th]
  \begin{minipage}{.5\textwidth}
    \begin{tabular}{l}
  \rel{\_OperatorAt}{insn, operator}.\\
  \rel{\_DummyIfVar}{insn, var}.\\
  \rel{\_AssignOperFromConstant}{insn,position,from}.\\
  \rel{\_IfConstant}{insn, position, constant}.\\\\\\
\end{tabular}
...
\begin{minipage}{.5\textwidth}
  \begin{tabular}{l}
  \rel{AssignOper\_From}{insn, position, from}.\\
  \rel{AssignOper\_FromConstant}{insn,position,from}\\
  \rel{If\_Var}{insn, position, var}.\\
  \rel{If\_Constant}{insn, position, constant}.\\
  \rel{DummyIf\_Var}{insn, var}.\\
  \rel{Operator\_At}{insn, operator}.
\end{tabular}
...
We provide here a brief presentation of the \emph{main} declared relations that make our
...
These relations and their associated rules are going to be thoroughly described in the
...
\begin{minipage}{.5\textwidth}
  ...
  \tab \rel{AssignLocal\_From}{?insn, ?param},\\
  \tab \rel{FormalParam}{\_, ?meth, ?param},\\
  \tab \rel{Var\_Type}{?param, ?type},\\
  ...
  \tab \record{?expr}\singlefieldbrackets{?param, ?type, nil, nil}.
\end{tabular}
...
\begin{minipage}{.5\textwidth}
  \begin{tabular}{l}
  \rel{ResolveExpr}{?meth, ?var, ?expr} \rulearrow\\
  \tab \rel{isMethodInvocation}{?insn},\\
  \tab \rel{MethodInvocation\_Method}{?insn, ?sig},\\
  \tab \rel{Method\_ReturnType}{?sig, \_, ?type},\\
  \tab (\rel{isPrimitiveType}{?type} ;\\
  \tab \rel{isReferenceType}{?type}),\\
  \tab \rel{Instruction\_Method}{?insn, ?meth},\\
  \tab \rel{AssignReturnValue}{?insn, ?var},\\
  \tab \record{?expr}\singlefieldbrackets{?param, ?type, nil, nil}.\\\\
\end{tabular}
...
\begin{figure}[th]
  \begin{minipage}{.5\textwidth}
    \begin{tabular}{l}
    \rel{ResolveExpr}{?meth, ?var, ?expr} \rulearrow\\
    \tab \rel{isLoadInstanceField\_Insn}{?insn},\\
\tab    \rel{LoadInstanceField\_To}{?insn, ?var},\\
\tab    \rel{Var\_Type}{?var, ?type},\\
\tab    (\rel{isPrimitiveType}{?type} ;\\ \tab \rel{isReferenceType}{?type}),\\
\tab    \rel{Instruction\_Method}{?insn, ?meth},\\
\tab    \record{?expr}\singlefieldbrackets{?var, ?type, nil, nil}.
\end{tabular}
...
\tab      \rel{isReferenceType}{?type}),\\
\tab    \record{c}\singlefield{count: \rel{AssignInstruction\_To}{\_, ?var}},\\
\tab    \recordname{c}\singlefield{ = 1}, \rel{AssicnLocal\_From}{?insn, ?from},\\
\tab \rel{ResolveExpr}{?meth, ?from, ?expr}.\\
  \end{tabular}
  \end{minipage}
  \begin{minipage}{.46\textwidth}
    \begin{tabular}{l}
    \rel{ResolveExpr}{?meth, ?var, ?expr} \rulearrow\\
\tab    \rel{isAssignUnop\_Insn}{?insn},\\
\tab    \rel{AssignInstruction\_To}{?insn, ?var},\\
\tab    \rel{Var\_Type}{?var, ?type},\\
\tab    \rel{isPrimitiveType}{?type} ;\\
\tab      \rel{isReferenceType}{?type}),\\
\tab    \rel{Instruction\_Method}{?insn, ?meth},\\
\tab    \rel{AssignOper\_From}{?insn, \_, ?right},\\
\tab    \rel{\_OperatorAt}{?insn, ?op},\\
\tab    \rel{ResolveExpr}{?meth, ?right, ?rExpr},\\
\tab    \record{?expr}\singlefieldbrackets{?op, ?type, ?rExpr, nil}.
  \end{tabular}
  \end{minipage}
...
\begin{figure}[th]
  \begin{equation}
    ...
    \end{aligned}
  \end{equation}
  \captionof{figure}{Path predicate example}
...
\section{Boolean Symbolic Reasoning}\label{s:symbolic}
...
\begin{minipage}{1.\textwidth}
     \begin{tabular}{l}
     \rel{ExprImpliesOther}{?expr, ?expr} \rulearrow \rel{isBooleanExpr}{?expr}.
    \end{tabular}
...
As seen above, we have encoded our tool's core reasoning within a set of few Datalog rules.
...
the scope of our reasoner, such as the theory of arithmetic. It is also worth mentioning that
in the context of this text we have not included several implementation details. The whole implemnetation
may be found in \doop{}'s public Github repository \cite{SOURCE} under the \emph{symbolic-reasoning} addon logic.
